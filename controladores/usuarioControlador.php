<?php
	...
	class usuarioControlador extends usuarioModelo{
		...
		public function paginador_usuario_controlador($pagina,$registros,$privilegio,$id,$url,$busqueda){
			...
			$tabla="";
			/**plantiamos una condicion con () ? : */
			$pagina=(isset($pagina) && $pagina > 0) ? (int) $pagina : 1 ;
			$inicio=($pagina>0) ? (($pagina*$registros)-$registros) : 0 ;
			/** isset se refire si biene definido */

			if(isset($busqueda) && $busqueda!=""){
				$consulta="SELECT  SQL_CALC_FOUND_ROWS * FROM usuario WHERE ((usuario_id!='$id'
				AND usuario_id!='1') AND (usuario_dni LIKE '%$busqueda%' OR usuario_nombre LIKE '%$busqueda%'
				OR usuario_apellido LIKE '%$busqueda%' OR usuario_telefono LIKE '%$busqueda%' OR
				usuario_email LIKE '%$busqueda%' OR usuario_usuario LIKE '%$busqueda%')) ORDER BY 
			...
			<table class="table table-dark table-sm">
				...
				if($total>=1 && $pagina<=$Npaginas){
					...
					if($total>=1){
						$tabla.='<tr class="text-center">
								<td colspan=9><a href="'.$url.'" class="btn btn-raised btn-primary btn-sm">Haga clic aca para recargar el listado</a></td>
							</tr>';
				}else{
					$tabla.='<tr class="text-center">
		...
		public function eliminar_usuario_controlador(){
				...
				if($check_prestamos->rowCount()>0){
					$alerta=[
						"Alerta"=>"simple",
						"Titulo"=>"Ocurrió un error inesperado",
						"Texto"=>"No podemos eliminar este usuario debido a que cuenta con prestamos asociados, recomendamos deshabilitar el usuario si ya no sera utilizado ",
						"Tipo"=>"error"
					];
					echo json_encode($alerta);
					exit();
				...
				if($eliminar_usuario->rowCount()==1){
					$alerta=[
						...
						"Texto"=>"El usuario a sido elminado del sistema exitosamente",
						"Tipo"=>"success"
					];
				}else{
					$alerta=[
		...
		public function actualizar_usuario_controlador(){
			...
			if($estado!="Activa" && $estado!="Deshabilitada"){
				$alerta=[
					"Alerta"=>"simple",
					"Titulo"=>"Ocurrió un error inesperado",
					"Texto"=>"El estado de la cuenta no coincide con el formato soliciatdo",
					"Tipo"=>"error"
				];
				echo json_encode($alerta);
				exit();
			...
			if($email!=$campos['usuario_email'] && $email!=""){
				...
				/*SI LA RESPUESTA ES VERDADERA ENTONCES EL CORREO QUE ESTA ENVIANDO DEL FORMULARIO ES UN CORREO VALIDO*/
				if(filter_var($email,FILTER_VALIDATE_EMAIL)){
					$check_email=mainModel::ejecutar_consulta_simple("SELECT usuario_email FROM usuario WHERE usuario_email='$email'");
				if($check_email->rowCount()>0){
				$alerta=[
